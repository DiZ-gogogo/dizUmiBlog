...
// src/prisma/supabase.ts
let import_supabase_js = require("@supabase/supabase-js");
let supabaseUrl = "https://qjfhlavwpqapvoyztkaz.supabase.co";
let supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFqZmhsYXZ3cHFhcHZveXp0a2F6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzIxNzg4MDEsImV4cCI6MjA0Nzc1NDgwMX0.9_Z8SJtBUZfL9yfM1KmIanMYqyID68KULNUO-ul15bU";
let supabase = (0, import_supabase_js.createClient)(supabaseUrl, supabaseKey);
async function fetchDataFromTable(tableName, options) {
...
let responseHandler = (res, statusCode, data = {}, message = "Network Error") => {
  const response = {
    status: statusCode,
    message,
    data
...
async function register_default(req, res) {
  try {
    ...
    if (!nickname && !password) {
      return responseHandler(res, 4e3, {}, "\u6635\u79F0\u548C\u5BC6\u7801\u4E0D\u80FD\u4E3A\u7A7A");
    }
    const tableName = "user";
    const existingUsers = await fetchDataFromTable(tableName);
    ...
    const { data, error } = await supabase_default.from(tableName).insert([newUser]);
    if (error) {
      return responseHandler(res, 5e3, {}, "\u6CE8\u518C\u5931\u8D25");
    }
    return responseHandler(res, 2e3, data, "\u6CE8\u518C\u6210\u529F");
  } catch (err) {
