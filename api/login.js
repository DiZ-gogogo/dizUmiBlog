  ...
  try {
    ...
    if (req.query.nickname && req.query.password) {
      const filteredData = data.filter((item) => {
        return item.nickname === req.query.nickname && item.password === req.query.password;
      });
      if (filteredData.length === 0) {
        return responseHandler(res, 4e3, {}, "token is invalid");
      }
      const user = filteredData[0];
      const { password, ...rest } = user;
      console.log(password);
      return responseHandler(res, 2e3, rest, "success");
    } else {
      return responseHandler(res, 4e3);
    }
  } catch (err) {
